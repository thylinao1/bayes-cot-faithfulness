...
**Two of those four holds are lifted.** A two-path equivalence gate (job 828507, Qwen3-8B, 30
...
eligible as cells of record; ruling R12(6) then removed OLMo-3-7B-Think and
DeepSeek-R1-Distill-Llama-8B from the hold list. The first completed off-mode cell
(Olmo-3-7B-Think, ARC-Challenge x metadata, job 828721) reaches 481/570 = 0.844 clean-correct
with every arm scorable, against the 1,381/1,500 unparseable this model returned under the
default reasoning mode above. Phi-4-reasoning's exploratory off-mode cell (job 828679, n = 570)
reaches 493/570 = 0.865 clean-correct, well above the 350 floor; its curves and arms are still
being read and R12(3) has not ruled yet. Sources: `docs/HOLDS-LIFT-PLAN.md` for the gate and the
lift, the operator's DECISION-LOG for the two cells' completion numbers (828721, 828679), not
yet mirrored into a repo document of its own.
...
kept as history below: the wider 18-cell pass further down supersedes it, and its ANCHORED
...
#### 18 cells of record: the fits pass that supersedes wave 1

A second, wider fits pass ran on 2026-09-08 and is now the table of record for column A and
column B on real models: 12 ARC-Challenge cells across four cue families (stated-hint,
professor, metadata, grader-code) plus 6 AQuA-RAT cells (stated-hint, professor), crossed with
three models, Qwen3-8B, Gemma-2-9B-it and Llama-3.1-8B-Instruct. The offset-null gate ran
first, as element 12 requires: attempt 1, seed 731, n = 10,000, verdict **PASS**, before any of
the 18 cells were fitted. Poisson-offset null: NDE -0.008062, NIE -0.000265, TE -0.008328
against a randomized arm difference of -0.008200. Gaussian-offset null: NDE -0.021037, NIE
+0.000027, TE -0.021010 against -0.021000. Both nulls converged and the fitted mediator mean
equals the control-arm mean exactly. Sources: `docs/CELLS18-FITS.md` and
`experiments/results/cells18-fits/offset_null_gate.json`, generated by
`experiments/cells18_fits_report.py` from the 18 cells' own `fit.json` and `model_row.json`
artifacts; no number below is typed by hand.

**Column B, model level: NDE, NIE, TE at rho = 0.** Section 2.4's estimand is the hierarchical
hyperparameter posterior across each model's own six cells, never an average of the six cell
...
|---|---|---|---|---|
| +0.1166 [+0.0298, +0.2139] | +0.0250 [-0.0342, +0.1434] | +0.1413 [+0.0346, +0.3491] | 0.015 | unresolved |

Gemma-2-9B-it, 6 cells, 5,544 items:

| NDE | NIE | TE | P(NIE > 0.15) | Verdict |
|---|---|---|---|---|
| +0.1262 [+0.0353, +0.2043] | +0.0523 [-0.0370, +0.2206] | +0.1770 [+0.0425, +0.3964] | 0.102 | unresolved |

Llama-3.1-8B-Instruct, 6 cells, 5,663 items:

| NDE | NIE | TE | P(NIE > 0.15) | Verdict |
|---|---|---|---|---|
| +0.0966 [+0.0101, +0.2250] | +0.0161 [-0.0353, +0.1160] | +0.1146 [+0.0130, +0.3028] | 0.012 | unresolved |

Sampler health, stated because it bears on how to read the intervals above: the hierarchical
fit carries six group deviations and two zero-centred cue-family deviations over a design whose
clean arm has no outcome variation, and it does not sample cleanly everywhere. Qwen3-8B: 130
divergences, max r_hat 1.010, minimum bulk ESS 495. Gemma-2-9B-it: 153 divergences, max r_hat
1.010, minimum bulk ESS 309. Llama-3.1-8B-Instruct: 72 divergences, max r_hat 1.000, minimum
bulk ESS 1,504. Source: `experiments/results/cells18-fits/<model>/model_row.json`.

**Claim status: 0 ANCHORED, 18 RAW.** 9 of the 18 cells pass their own per-cell anchor test (all
...
so all 18 cells hold at RAW.

**The tension this pass surfaces.** The pooled replay-anchor NIE contrast is large and tight for
every model: +0.1458 [+0.1358, +0.1555] for Qwen3-8B, +0.1686 [+0.1599, +0.1799] for
Gemma-2-9B-it, +0.2176 [+0.2089, +0.2282] for Llama-3.1-8B-Instruct. The fitted model-level NIE
above is small for every model, with an interval that covers zero in all three cases. Read
plainly, on every model here the randomized anchor and the mediation fit disagree about how much
of the hint effect actually travels with the written chain: the anchor says a large share of it
does, the fit says the mediated path is not distinguishable from noise. That disagreement, not
either number on its own, is what this pass finds.

**The logit-level column B (Amendment A5) is not printed for any of the 18 cells.** Its
eligibility gate fails the same condition everywhere: no arms record in this project carries
`intervention_level = logit`, because the generation pass that would write one (Job B of
`docs/OUTCOME-SCALE-NOTE.md` part 4.5) has not run yet. That pass is being built rather than
skipped.
...
predates the 2026-09-07 prior repair and is logit), the rule for pooling the anchor across a
model's cells, and the fact that the model-level agreement test pairs a posterior against a
bootstrap on independent draws rather than one paired resample, are all lane choices, each named
in `model_row.json` under `choices_that_make_this_provisional` so a later lane can change any one
of them without rediscovering it.

One of six sensitivity fits (a logit-link and a substrate-grouped refit per model) did not mix:
Qwen3-8B's substrate-grouped fit, at max r_hat 1.080, 353 divergences and a minimum bulk ESS of
44. It is printed as a failure and read neither as support for nor against the primary row
beside it.

Full tables, every cell's own block, and the 18-cell index: `docs/CELLS18-FITS.md`.
...
| qwen3-32b | not run yet | pending | row submitted (job 828754), queued behind the a100-80 pool |
...
- `docs/WAVE1-FITS.md`: column A and column B on the three usable cells, the anchor, claim status (superseded by `docs/CELLS18-FITS.md` below, kept as history).
- `docs/CELLS18-FITS.md`: the 18-cell pass of record, column A and column B on all 18 cells, the model-level rows and why they are PROVISIONAL, and the claim-status mix (0 ANCHORED, 18 RAW).
...
- `docs/JURY-Q1D-GATE.md`: the prompt-d construct revision ruling R9 asked for, on Llama, Gemma and gpt-oss-20b (two budgets), Qwen pending.
...
- `docs/HOLDS-LIFT-PLAN.md`: the two-path gate, the holds lifted for OLMo-3-7B-Think and DeepSeek-R1-Distill-Llama-8B, and the Phi-4-reasoning exploratory off cell.
