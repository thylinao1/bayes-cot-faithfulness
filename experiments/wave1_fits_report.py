...
def header(gate: dict, fits: dict) -> list[str]:
    ...
    return [
        ...
        f"**Code commit at run time:** `{d['code_commit']}` (the repository state the cluster",
        "job was submitted against; the analysis script's own content is pinned by the hash",
        "below and is what the next commit on this branch contains)",
